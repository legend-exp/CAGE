...
class SlackInterface(Gogol):
    ...
    def __init__(self,
        ...
        slack = {}
        config_file = json.loads(open(this_home+'/.cage_config.json').read())
        if 'slack' in config_file:
            slack = config_file['slack']
        else:
            raise dripline.core.exceptions.DriplineValueError('Warning! unable to find slack credentials in <~/.cage_config.json>')
        ...
        if token:
            ...
            raise dripline.core.exceptions.DriplineValueError('Warning! unable to find slack token in <~/.cage_config.json>')
